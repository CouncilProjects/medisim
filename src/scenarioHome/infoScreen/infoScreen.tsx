import { Center, Stack, Text, Paper, ScrollArea, Box, Group, Badge, Modal, Button } from '@mantine/core';
...
export default function InfoScreen() {
  ...
  useEffect(() => {
    let stuff = context.getMedicalSituation();
    console.log(stuff);
    setEvents(stuff);
            ...
            >
              <Group justify='space-between'>
                <Stack flex={5}>
                  ...
                  >
                    Events:  {events.length - viewEvent} of {events.length}
                  </Text>
                  <Text
                    size="md"
                    fw={500}
                ...
                <Stack flex={1}>
                  <Button variant='light' disabled={viewEvent == 0} onClick={() => { setViewEvent((prev) => Math.max(prev - 1, 0)) }}>
                    <IconArrowNarrowUp></IconArrowNarrowUp>
                  </Button>
                  <Button variant='light' disabled={viewEvent == events.length - 1} onClick={() => { setViewEvent((prev) => Math.min(prev + 1, events.length)) }}>
